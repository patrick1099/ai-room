...
- [ ] 主聊发起文档审查，只把一个精确文档加入 `--writable-doc`；顾问仅修改该文件且 reply
  ...
  - 结果：[ ] Pass [ ] Fail
  - 证据/备注：
- [ ] 新发起一项任务，在顾问取得任务后尝试改变一个源码文件；确认顾问即使请求 `done`，
  reply 也被降级为 `blocked`、`guard_violations` 列出源码路径、正文说明无法判定改动来源，
  且工具没有自动恢复或删除该文件。
  - 测试后由主聊/用户恢复的源码路径：
  - 结果：[ ] Pass [ ] Fail
  - 证据/备注：
- [ ] 顾问取得任务后，由主聊窗口或你自己改动工作树里的文件；确认顾问的 reply 仍然送达主聊
  （降级为 `blocked` 并附路径），信道没有卡死。
  - 结果：[ ] Pass [ ] Fail
  - 证据/备注：
